...
function registerDocumentTools(server) {
  ...
  server.tool(
    ...
    {
      channel: import_zod.z.string().describe("The name of the channel to join")
    },
    async ({ channel }) => {
      try {
        if (!channel) {
  ...
  server.tool(
    ...
    {
      pageId: import_zod.z.string().describe("ID of the page to delete")
    },
    async ({ pageId }) => {
      try {
  ...
  server.tool(
    ...
    {
      pageId: import_zod.z.string().describe("ID of the page to rename"),
      name: import_zod.z.string().describe("New name for the page")
    },
    async ({ pageId, name }) => {
      try {
  ...
  server.tool(
    ...
    {
      pageId: import_zod.z.string().describe("ID of the page to switch to")
    },
    async ({ pageId }) => {
      try {
  ...
  server.tool(
    ...
    {
      pageId: import_zod.z.string().describe("ID of the page to duplicate"),
      name: import_zod.z.string().optional().describe("Optional name for the duplicated page (defaults to 'Original Name (Copy)')")
    },
    async ({ pageId, name }) => {
      try {
...
function registerCreationTools(server) {
  server.tool(
    ...
    {
      x: import_zod2.z.number().describe("X position (local coordinates, relative to parent)"),
      y: import_zod2.z.number().describe("Y position (local coordinates, relative to parent)"),
  ...
  server.tool(
    ...
    "Create a new text element in Figma",
    {
      ...
      textAlignHorizontal: import_zod2.z.enum(["LEFT", "CENTER", "RIGHT", "JUSTIFIED"]).optional().describe("Horizontal text alignment. Use RIGHT for Arabic/RTL text."),
      textAutoResize: import_zod2.z.enum(["WIDTH_AND_HEIGHT", "HEIGHT", "NONE", "TRUNCATE"]).optional().describe("Text resize behavior. Use HEIGHT for fixed-width text that wraps."),
      width: import_zod2.z.number().positive().optional().describe("Fixed width for the text node. Use with textAutoResize HEIGHT for wrapping text within a specific width.")
    },
  ...
  server.tool(
    ...
    "Clone an existing node in Figma",
    {
      ...
      x: import_zod2.z.number().optional().describe("New X position for the clone (local coordinates, relative to parent)"),
      y: import_zod2.z.number().optional().describe("New Y position for the clone (local coordinates, relative to parent)")
    },
    async ({ nodeId, x, y }) => {
      try {
  ...
  server.tool(
    ...
    {
      nodeIds: import_zod2.z.array(import_zod2.z.string()).min(2).describe("Array of node IDs to combine (minimum 2). Order matters for SUBTRACT."),
      operation: import_zod2.z.enum(["UNION", "SUBTRACT", "INTERSECT", "EXCLUDE"]).describe("Boolean operation type"),
      name: import_zod2.z.string().optional().describe("Optional name for the resulting node")
    },
...
function registerModificationTools(server) {
  ...
  server.tool(
    ...
    "Set the stroke color of a node in Figma (defaults: opacity 1, weight 1)",
    {
      ...
      strokeWeight: import_zod3.z.number().min(0).optional().describe("Stroke weight >= 0)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to modify (typically an icon instance)"),
      r: import_zod3.z.number().min(0).max(1).describe("Red component (0-1)"),
      g: import_zod3.z.number().min(0).max(1).describe("Green component (0-1)"),
      b: import_zod3.z.number().min(0).max(1).describe("Blue component (0-1)"),
      a: import_zod3.z.number().min(0).max(1).optional().describe("Alpha component (0-1, defaults to 1)")
    },
    async ({ nodeId, r, g, b, a }) => {
      try {
        if (r === void 0 || g === void 0 || b === void 0) {
  ...
  server.tool(
    ...
    "Move a node to a new position in Figma",
    {
      ...
      x: import_zod3.z.number().describe("New X position (local coordinates, relative to parent)"),
      y: import_zod3.z.number().describe("New Y position (local coordinates, relative to parent)")
    },
    async ({ nodeId, x, y }) => {
      try {
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to rotate"),
      angle: import_zod3.z.number().describe("Rotation angle in degrees (clockwise)"),
      relative: import_zod3.z.boolean().optional().describe("If true, add angle to current rotation instead of setting absolute value (default: false)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to modify"),
      visible: import_zod3.z.boolean().optional().describe("Set node visibility (true = visible, false = hidden)"),
      locked: import_zod3.z.boolean().optional().describe("Set node lock state (true = locked, false = unlocked)"),
      opacity: import_zod3.z.number().min(0).max(1).optional().describe("Set node opacity (0 = fully transparent, 1 = fully opaque)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to reorder"),
      position: import_zod3.z.enum(["front", "back", "forward", "backward"]).optional().describe("Move to front/back or one step forward/backward"),
      index: import_zod3.z.number().optional().describe("Direct index position within parent's children (0 = bottom). Overrides position if both provided.")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to convert to a frame")
    },
    async ({ nodeId }) => {
      try {
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to modify"),
      type: import_zod3.z.enum(["GRADIENT_LINEAR", "GRADIENT_RADIAL", "GRADIENT_ANGULAR", "GRADIENT_DIAMOND"]).describe("Gradient type"),
      stops: import_zod3.z.array(import_zod3.z.object({
        position: import_zod3.z.number().min(0).max(1).describe("Stop position (0-1, where 0 is start and 1 is end)"),
        color: import_zod3.z.object({
          r: import_zod3.z.number().min(0).max(1).describe("Red (0-1)"),
          g: import_zod3.z.number().min(0).max(1).describe("Green (0-1)"),
          b: import_zod3.z.number().min(0).max(1).describe("Blue (0-1)"),
          a: import_zod3.z.number().min(0).max(1).optional().describe("Alpha (0-1, defaults to 1)")
        })
      })).min(2).describe("Array of gradient color stops (minimum 2)"),
      gradientTransform: import_zod3.z.array(import_zod3.z.array(import_zod3.z.number())).optional().describe("2x3 affine transform matrix [[a,b,tx],[c,d,ty]]. Defaults to left-to-right linear: [[1,0,0],[0,1,0]]")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to apply the image fill to"),
      imageData: import_zod3.z.string().max(7e6).describe("Base64-encoded image data (PNG, JPEG, GIF, or WebP). Max ~5MB after decode."),
      scaleMode: import_zod3.z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().describe("How the image is scaled within the node (default: FILL)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the frame node to apply grids to"),
      grids: import_zod3.z.array(
        import_zod3.z.object({
          pattern: import_zod3.z.enum(["COLUMNS", "ROWS", "GRID"]).describe("Grid pattern type"),
          count: import_zod3.z.number().optional().describe("Number of columns/rows (ignored for GRID)"),
          sectionSize: import_zod3.z.number().optional().describe("Size of each section in pixels"),
          gutterSize: import_zod3.z.number().optional().describe("Gutter size between sections in pixels"),
          offset: import_zod3.z.number().optional().describe("Offset from the edge in pixels"),
          alignment: import_zod3.z.enum(["MIN", "CENTER", "MAX", "STRETCH"]).optional().describe("Grid alignment"),
          visible: import_zod3.z.boolean().optional().describe("Whether the grid is visible (default: true)"),
          color: import_zod3.z.object({
            r: import_zod3.z.number().min(0).max(1).describe("Red (0-1)"),
            g: import_zod3.z.number().min(0).max(1).describe("Green (0-1)"),
            b: import_zod3.z.number().min(0).max(1).describe("Blue (0-1)"),
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the frame node to read grids from")
    },
    async ({ nodeId }) => {
      try {
  ...
  server.tool(
    ...
    {
      pageId: import_zod3.z.string().describe("The ID of the page to add guides to"),
      guides: import_zod3.z.array(
        import_zod3.z.object({
          axis: import_zod3.z.enum(["X", "Y"]).describe("Guide axis: X for vertical, Y for horizontal"),
          offset: import_zod3.z.number().describe("Offset position of the guide in pixels")
        })
  ...
  server.tool(
    ...
    {
      pageId: import_zod3.z.string().describe("The ID of the page to read guides from")
    },
    async ({ pageId }) => {
      try {
  ...
  server.tool(
    ...
    "Add an annotation label to a node in Figma. Uses the proposed Annotations API \u2014 requires Figma Desktop with enableProposedApi.",
    {
      ...
      label: import_zod3.z.string().describe("The annotation label text")
    },
    async ({ nodeId, label }) => {
      try {
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to read annotations from")
    },
    async ({ nodeId }) => {
      try {
  ...
  server.tool(
    ...
    {
      nodeId: import_zod3.z.string().describe("The ID of the node to rename"),
      name: import_zod3.z.string().describe("The new name for the node")
    },
    async ({ nodeId, name }) => {
      try {
...
${failedResults.map(
  ...
  server.tool(
    ...
    {
      nodeId: import_zod4.z.string().describe("The ID of the text node to modify"),
      textStyleId: import_zod4.z.string().describe("The ID of the text style to apply")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod4.z.string().describe("The ID of the text node to modify"),
      textAlignHorizontal: import_zod4.z.enum(["LEFT", "CENTER", "RIGHT", "JUSTIFIED"]).optional().describe("Horizontal text alignment (LEFT, CENTER, RIGHT, JUSTIFIED). Use RIGHT for Arabic/RTL text."),
      textAlignVertical: import_zod4.z.enum(["TOP", "CENTER", "BOTTOM"]).optional().describe("Vertical text alignment (TOP, CENTER, BOTTOM)")
    },
...
function registerComponentTools(server) {
  server.tool(
    ...
    "Create an instance of a component in Figma",
    {
      ...
      x: import_zod5.z.number().describe("X position (local coordinates, relative to parent)"),
      y: import_zod5.z.number().describe("Y position (local coordinates, relative to parent)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod5.z.string().describe("The ID of the node to convert into a component"),
      name: import_zod5.z.string().optional().describe("Optional new name for the component")
    },
    async ({ nodeId, name }) => {
      try {
  ...
  server.tool(
    ...
    {
      componentIds: import_zod5.z.array(import_zod5.z.string()).describe("Array of component node IDs to combine into a component set"),
      name: import_zod5.z.string().optional().describe("Optional name for the component set")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod5.z.string().describe("The ID of the instance node to modify"),
      properties: import_zod5.z.record(import_zod5.z.string()).describe('Variant properties to set as key-value pairs (e.g., { "State": "Hover", "Size": "Large" })')
    },
...
function registerImageTools(server) {
  server.tool(
    ...
    {
      nodeId: import_zod6.z.string().describe("The ID of the node to apply image to"),
      imageSource: import_zod6.z.string().describe("Image URL or base64 data string"),
      sourceType: import_zod6.z.enum(["url", "base64"]).describe("Source type: 'url' for image URL, 'base64' for base64 encoded data"),
      scaleMode: import_zod6.z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().describe("Image scaling mode (default: FILL)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod6.z.string().describe("The ID of the node to get image from")
    },
    async ({ nodeId }) => {
      try {
...
- Filters: ${typedResult.filters ? JSON.stringify(typedResult.filters) : "none"}`
  ...
  server.tool(
    ...
    {
      nodeId: import_zod6.z.string().describe("The ID of the node with image to replace"),
      newImageSource: import_zod6.z.string().describe("New image URL or base64 data"),
      sourceType: import_zod6.z.enum(["url", "base64"]).describe("Source type: 'url' or 'base64'"),
      preserveTransform: import_zod6.z.boolean().optional().describe("Preserve existing image transform (default: true)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod6.z.string().describe("The ID of the node to transform image on"),
      scaleMode: import_zod6.z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().describe("Change scale mode"),
      rotation: import_zod6.z.union([import_zod6.z.literal(0), import_zod6.z.literal(90), import_zod6.z.literal(180), import_zod6.z.literal(270)]).optional().describe("Rotation in 90-degree increments (0, 90, 180, 270). Rotates the IMAGE inside the node, not the node itself."),
      translateX: import_zod6.z.number().optional().describe("Horizontal translation offset"),
      translateY: import_zod6.z.number().optional().describe("Vertical translation offset"),
      scale: import_zod6.z.number().positive().optional().describe("Scale factor (1 = 100%)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod6.z.string().describe("The ID of the node with image fill"),
      exposure: import_zod6.z.number().min(-1).max(1).optional().describe("Brightness adjustment (-1.0 to 1.0)"),
      contrast: import_zod6.z.number().min(-1).max(1).optional().describe("Contrast adjustment (-1.0 to 1.0)"),
      saturation: import_zod6.z.number().min(-1).max(1).optional().describe("Color intensity (-1.0 to 1.0, -1 = grayscale)"),
      temperature: import_zod6.z.number().min(-1).max(1).optional().describe("Warm/cool tint (-1.0 to 1.0)"),
      tint: import_zod6.z.number().min(-1).max(1).optional().describe("Green/magenta shift (-1.0 to 1.0)"),
      highlights: import_zod6.z.number().min(-1).max(1).optional().describe("Bright area adjustment (-1.0 to 1.0)"),
      shadows: import_zod6.z.number().min(-1).max(1).optional().describe("Dark area adjustment (-1.0 to 1.0)")
    },
...
function registerSvgTools(server) {
  server.tool(
    ...
    {
      svgString: import_zod7.z.string().max(5e5).describe("SVG markup string (max 500KB). Must contain a valid <svg> element."),
      x: import_zod7.z.number().optional().describe("X position for the imported SVG (default: 0)"),
      y: import_zod7.z.number().optional().describe("Y position for the imported SVG (default: 0)"),
      name: import_zod7.z.string().optional().describe("Optional name for the imported node"),
      parentId: import_zod7.z.string().optional().describe("Optional parent node ID to place the SVG into")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod7.z.string().describe("The ID of the node to export as SVG")
    },
    async ({ nodeId }) => {
      try {
...
function registerVariableTools(server) {
  ...
  server.tool(
    ...
    {
      collectionId: import_zod8.z.string().optional().describe("ID of an existing variable collection"),
      collectionName: import_zod8.z.string().optional().describe("Name for a new collection (used if collectionId not provided)"),
      name: import_zod8.z.string().describe("Variable name"),
      resolvedType: import_zod8.z.enum(["COLOR", "FLOAT", "STRING", "BOOLEAN"]).describe("Variable type"),
      value: import_zod8.z.any().describe("Variable value. COLOR: {r,g,b,a} (0-1). FLOAT: number. STRING: string. BOOLEAN: boolean."),
      modeId: import_zod8.z.string().optional().describe("Mode ID to set the value for (uses default mode if omitted)")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod8.z.string().describe("The ID of the node to bind the variable to"),
      variableId: import_zod8.z.string().describe("The ID of the variable to bind"),
      field: import_zod8.z.string().describe("The node property field to bind (e.g., 'fills/0/color', 'opacity', 'width', 'height')")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod8.z.string().describe("The ID of the node to switch mode on"),
      collectionId: import_zod8.z.string().describe("The ID of the variable collection"),
      modeId: import_zod8.z.string().describe("The ID of the mode to switch to")
    },
...
function registerFigJamTools(server) {
  ...
  server.tool(
    ...
    {
      x: import_zod9.z.number().describe("X position on the canvas"),
      y: import_zod9.z.number().describe("Y position on the canvas"),
      text: import_zod9.z.string().describe("Text content of the sticky note"),
      color: import_zod9.z.enum([
        "yellow",
        "pink",
        "green",
        "blue",
      ...
      isWide: import_zod9.z.boolean().optional().describe("Whether the sticky note should be wide format (default: false)"),
      name: import_zod9.z.string().optional().describe("Optional name/label for the node"),
      parentId: import_zod9.z.string().optional().describe("Optional parent node ID (e.g. a section) to place the sticky into")
    },
  ...
  server.tool(
    ...
    {
      nodeId: import_zod9.z.string().describe("The ID of the sticky note node to update"),
      text: import_zod9.z.string().describe("The new text content")
    },
    async ({ nodeId, text }) => {
      try {
  ...
  server.tool(
    ...
    "Create a FigJam shape with text inside. Useful for flowcharts, diagrams, and process maps. Supported shapes: SQUARE, ELLIPSE, ROUNDED_RECTANGLE, DIAMOND, TRIANGLE_UP, TRIANGLE_DOWN, PARALLELOGRAM_RIGHT, PARALLELOGRAM_LEFT.",
    {
      x: import_zod9.z.number().describe("X position on the canvas"),
      y: import_zod9.z.number().describe("Y position on the canvas"),
      width: import_zod9.z.number().optional().describe("Width of the shape (default: 200)"),
      height: import_zod9.z.number().optional().describe("Height of the shape (default: 200)"),
      shapeType: import_zod9.z.enum([
        "SQUARE",
        "ELLIPSE",
        "ROUNDED_RECTANGLE",
        "DIAMOND",
      ...
      text: import_zod9.z.string().optional().describe("Text to display inside the shape"),
      fillColor: import_zod9.z.object({
        r: import_zod9.z.number().min(0).max(1).describe("Red (0-1)"),
        g: import_zod9.z.number().min(0).max(1).describe("Green (0-1)"),
        b: import_zod9.z.number().min(0).max(1).describe("Blue (0-1)"),
        a: import_zod9.z.number().min(0).max(1).optional().describe("Alpha (0-1)")
      }).optional().describe("Fill color in RGBA format (0-1 range each component)"),
      name: import_zod9.z.string().optional().describe("Optional name for the node"),
      parentId: import_zod9.z.string().optional().describe("Optional parent node ID to place the shape into")
    },
  ...
  server.tool(
    ...
    {
      startNodeId: import_zod9.z.string().optional().describe("ID of the node where the connector starts (omit to use startX/startY)"),
      startX: import_zod9.z.number().optional().describe("X position of the connector start point (used when startNodeId is not provided)"),
      startY: import_zod9.z.number().optional().describe("Y position of the connector start point (used when startNodeId is not provided)"),
      endNodeId: import_zod9.z.string().optional().describe("ID of the node where the connector ends (omit to use endX/endY)"),
      endX: import_zod9.z.number().optional().describe("X position of the connector end point (used when endNodeId is not provided)"),
      endY: import_zod9.z.number().optional().describe("Y position of the connector end point (used when endNodeId is not provided)"),
      connectorLineType: import_zod9.z.enum(["ELBOWED", "STRAIGHT", "CURVED"]).optional().describe("Line routing style (default: ELBOWED)"),
      startStrokeCap: import_zod9.z.enum(["NONE", "ARROW", "ARROW_EQUILATERAL", "CIRCLE_FILLED", "DIAMOND_FILLED"]).optional().describe("Arrowhead at the start (default: NONE)"),
      endStrokeCap: import_zod9.z.enum(["NONE", "ARROW", "ARROW_EQUILATERAL", "CIRCLE_FILLED", "DIAMOND_FILLED"]).optional().describe("Arrowhead at the end (default: ARROW)"),
      strokeColor: import_zod9.z.object({
        r: import_zod9.z.number().min(0).max(1),
        g: import_zod9.z.number().min(0).max(1),
        b: import_zod9.z.number().min(0).max(1),
        a: import_zod9.z.number().min(0).max(1).optional()
      }).optional().describe("Stroke color in RGBA format"),
      strokeWeight: import_zod9.z.number().positive().optional().describe("Stroke weight / line thickness"),
      name: import_zod9.z.string().optional().describe("Optional name for the connector node")
    },
    async ({
      startNodeId,
      startX,
  ...
  server.tool(
    ...
    "Create a FigJam section. Sections are used to group and organise content on the FigJam board. They appear as labelled coloured regions.",
    {
      x: import_zod9.z.number().describe("X position on the canvas"),
      y: import_zod9.z.number().describe("Y position on the canvas"),
      width: import_zod9.z.number().optional().describe("Width of the section (default: 800)"),
      height: import_zod9.z.number().optional().describe("Height of the section (default: 600)"),
      name: import_zod9.z.string().optional().describe("Label / name for the section"),
      fillColor: import_zod9.z.object({
        r: import_zod9.z.number().min(0).max(1),
        g: import_zod9.z.number().min(0).max(1),
        b: import_zod9.z.number().min(0).max(1),
